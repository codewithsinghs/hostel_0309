...
const emits = defineEmits(["create", "custom-action"]);
  ...
  table = $("#resourceTable").DataTable({
    ...
    drawCallback: () => {
      ...
      $(".toggle-status").off().on("change", function () {
        ...
        axios.patch(`${props.resourceUrl}/${id}/status`, { status })
          .then(() => Swal.fire("Updated", "Status updated successfully", "success"))
...
const handleAction = (action, row) => {
  if (action === "view") {
    ...
    emits("custom-action", { action, row });
  }
};
...
<template>
  ...
  <!-- Placeholder modal for View/Edit -->
